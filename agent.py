...
# ⚙️ 2. 基本設定 ＆ グローバル変数定義
# ==================================================
CURRENT_VERSION = "2.2.0"  # 👈 バージョンアップ (Macro & Legal Guardrails Integrated)
...
async def handle_chat_message(ctx: Context, sender: str, msg: ChatMessage):
    ...
    reply_text = (
        f"🤖 13-Chain Unified Ledger RWA & Macro Intelligence Agent (Ver {CURRENT_VERSION}) です！\n"
        f"現在 13 チェーンの監視および主要金融・超国家機関（BRICS/WEF/SEC/Fed/Treasury/SSA等）の一次情報を自動照合中です。\n"
...
class DataQueryResponse(Model):
    ...
    disclaimer: str  # 👈 Not Financial Advice リーガル免責フィールド追加
...
KEYWORD_MAP = {
    ...
    "macro": ["house financial", "sec", "bis", "treasury", "brics", "wef", "ssa", "fed", "ecb"]
}
    ...
    for chain_name, chain_status in latest_chain_data.items():
        ...
        if matched_keywords:
            correlated_signals.append({
                ...
                "summary": f"On-chain activity for {chain_name.upper()} correlates with macro/regulatory updates ({', '.join(matched_keywords)}).",
                "timestamp": time.time()
            })
...
async def startup_handler(ctx: Context):
    ...
    send_discord_message(
        ...
        f"• 監視対象: 13-Chain + Macro/BRICS/WEF/SEC/Fed Intelligence\n"
...
async def check_and_update_task(ctx: Context):
    global last_checked_sepolia_block, latest_chain_data, latest_detected_signals
...
async def handle_dynamic_quote(ctx: Context, sender: str, msg: DataQueryRequest):
    ...
    if requested_target in ["full", "intelligence", "full_intelligence"]:
        quoted_price, desc = "3.0", "Institutional Grade Combined Intelligence (On-Chain + Global Macro/BRICS/WEF)"
...
async def handle_paid_delivery(ctx: Context, sender: str, msg: CommitPayment):
    ...
    response_data = DataQueryResponse(
        ...
        disclaimer=LEGAL_DISCLAIMER_TEXT  # 👈 免責文言の自動付与
    )
